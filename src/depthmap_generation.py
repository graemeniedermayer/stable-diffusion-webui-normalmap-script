...
def estimatepatchfusion(image, model, w, h):
    ...
    with torch.no_grad():
        # resizing
        # this all necessary?
        img = F.interpolate(torch.tensor(np.array([image])).unsqueeze(dim=0).permute(0, 3, 1, 2), (2160, 3840), mode='bicubic', align_corners=True)
        img = img.squeeze().permute(1, 2, 0).unsqueeze(dim=0).numpy()
